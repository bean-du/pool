...
func NewConnPool(opt *Options, logger Logger) *ConnPool {
	...
	if p.opt.ReceiveHandler != nil {
		go p.handleReceiveDate()
	}

	return p
}

func (p *ConnPool) handleReceiveDate() {
	...
	for {
		...
		case <-p.closedCh:
			return
		case data := <-p.receiver:
			p.opt.ReceiveHandler(context.Background(), data)
		}
	}
}
...
func (p *ConnPool) dialConn(ctx context.Context, pooled bool) (*Conn, error) {
	...
	if p.opt.Keepalive != nil {
		p.opt.Keepalive(cn.netConn)
	}

	return cn, err
}

func (p *ConnPool) addPoller(ctx context.Context, cn *Conn) error {
	var (
		err  error
